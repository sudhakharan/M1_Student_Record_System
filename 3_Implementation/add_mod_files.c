...
void window(){
    ...
    gotoxy(20,3);
    printf("Rajalakshmi Engineering College,Chennai,Tamil Nadu");
    gotoxy(31,4);
...
void gen_marksheet(){
    ...
    clearWindow();
    print_heading("Enter Total Marks");
    int print = 37;
    FILE *fp;
    fp = fopen("record.txt","rb+");
    SetColor(45);
    if(fp == NULL){
        ...
        //here you can confirms the ID
        gotoxy(print,12);printf("Name: ");gets(stu.name);
        gotoxy(print,13);printf("Marks obtained: ");gets(stu.mark);
        fwrite(&stu, sizeof(stu), 1, fp);
        gotoxy(40,22); printf("The mark is sucessfully added");
    }
    SetColor(28);
    fclose(fp);
    return;
...
void view_marks(){
    ...
    while(fread(&stu, sizeof(stu),1,fp) == 1){
        if(strcmp(s_id, stu.ID) == 0){
            ...
            gotoxy(print,12);printf("ID: ");gets(stu.ID);
            gotoxy(print,13);printf("Name: ");gets(stu.name);
            gotoxy(print,14);printf("Mark ");gets(stu.mark);
            fseek(fp,-sizeof(stu), SEEK_CUR);
            fwrite(&stu,sizeof(stu), 1, fp);
            isFound = 1;
            break;
    ...
    if(!isFound){
        gotoxy(print, 14);printf("Mark Found:",14);
    }
    fclose(fp);
    SetColor(28);
    return;
